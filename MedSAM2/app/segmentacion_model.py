...
FRAMES_OUTPUT = r"frames"
VIDEO_OUTPUT = r"video"
# if os.name == 'nt':
# MODEL_CONFIG = os.getcwd() + r"//sam2/configs/sam2.1_hiera_t512.yaml"
# MODEL_CHECKPOINT = os.getcwd() + r"//checkpoints/MedSAM2_latest.pt"
...
if platform.system() == "Windows":
    INPUT_FOLDER_STORAGE = Path(os.getcwd() + r"//inputs")
else:
...
def predict_masks_with_points(
    ...
    if len(os.listdir(input_folder_bwd)) > 0:
        ...
        for out_frame_idx, per_obj_output_mask in video_segments2.items():
            mask = get_mask(per_obj_output_mask, height, width)
            masks.append(
                {"frame": lowest_frame - out_frame_idx - 1, "mask": mask.tolist()}
            )
